...
class EpisodicPendulumEnv(gym.Wrapper):
    ...
    def step(self, action):
        obs, reward, done, _, infos = self.env.step(action)
        self.was_real_done = done
        # check current lives, make loss of life terminal,
        # then update lives to handle bonus lives
        self.lives += 1
        ...
        return obs, reward, done, _, infos
...
class GenerateFrame42(gym.ObservationWrapper):
    ...
    def step(self, u):
        obs, costs, done, _, infos = self.env.step(u)
        return self._get_obs(), costs, done, _, infos
...
class FrameStack(gym.Wrapper):
    ...
    def step(self, action):
        ob, reward, done, _, infos = self.env.step(action.squeeze())
        
        self.frames.append(ob[0])
        self.states.append(ob[1])
        return self._get_obs(), reward, done, _, infos
...
def wrap_cover_pendulun(env_name, disc_actions, render_mode=None, episode_length=200, full_state=False):
    def wrap_():
        """Apply a common set of wrappers for Atari games."""
        print("NAME: ", env_name)
        env = PendulumEnv(g=9.81, render_mode=render_mode, full_state=full_state)
...
def wrap_cover_pendulun_test(env_name, disc_actions, render_mode=None, episode_length=200, full_state=False):
    def wrap_():
        """Apply a common set of wrappers for Atari games."""
        print("NAME: ", env_name)
        env = PendulumEnv(g=9.81, render_mode=render_mode, full_state=full_state)
...
def wrap_cover_pendulun_conti(
    ...
    def wrap_():
        ...
        print("NAME: ", env_name)
        env = PendulumEnv(
            g=9.81, render_mode=render_mode, full_state=full_state, fixed=fixed, repeat=repeat)
        env = GenerateFrame42(env)
        env = ProcessFrame42(env)
        env = ImageToPyTorch(env)
        env = EpisodicPendulumEnv(env, episode_length=episode_length)
...
def wrap_cover_pendulun_super(env_name, render_mode=None, episode_length=200, full_state=False):
    def wrap_():
        """Apply a common set of wrappers for Atari games."""
        print("NAME: ", env_name)
        env = PendulumEnv(g=9.81, render_mode=render_mode, full_state=full_state)
