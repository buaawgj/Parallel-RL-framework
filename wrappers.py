...
class GenerateFrame84(gym.ObservationWrapper):
    def __init__(self, env=None):
        super(GenerateFrame84, self).__init__(env)
...
def wrap_cover_pendulun(env_name):
    def wrap_():
        """Apply a common set of wrappers for Atari games."""
        ...
        env = GenerateFrame84(env)
        env = ProcessFrame84(env)
        env = ImageToPyTorch(env)
        env = EpisodicPendulumEnv(env)
        env = FrameStack(env, 4)
