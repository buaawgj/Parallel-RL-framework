...
class Actor(nn.Module):
    def __init__(self, channel_num, state_dim, action_dim, max_action):
        ...
        self.feature_extraction = nn.Sequential(
            ...
            # 8 -> 10
            nn.Conv2d(channel_num, 8, kernel_size=8, stride=2),
            # nn.ReLU(),
...
class Critic(nn.Module):
    def __init__(self, channel_num, state_dim, action_dim):
        ...
        # Remove velocity information.
        self.fc_0 = nn.Linear(8 * 8 * 10 + N_TILE * action_dim, 400)
        # self.fc_0 = nn.Linear(8 * 8 * 10, 400)  
        self.fc_1 = nn.Linear(400, 300)
        # action value
...
class DDPG(object):
    def __init__(self, channel_num, state_dim, action_dim, max_action):
        ...
        self.critic_optimizer = optim.Adam(self.critic.parameters(), lr=9e-5)
    ...
    def choose_action(self, s, evaluate=False, explore=False):
        # x:state
        ...
        if explore:
            action = np.random.uniform(-self.max_action, self.max_action, size=(state.shape[0],))
            return action.reshape(state.shape[0],)
    ...
    def learn(self):
        ...
        if self.learn_step_counter % (TARGET_REPLACE_ITER * 2) == 0:
        ...
        epsilon = np.clip(0.2 * np.random.normal(size=next_target_a.shape), -0.35, 0.35)
        epsilon = torch.FloatTensor(epsilon).to(device)
        next_target_a = torch.clip(next_target_a + epsilon, -self.max_action, self.max_action)
